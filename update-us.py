...
def _yahoo_dividend_for(ticker: str, target_date: str):
    """Return the dividend-per-share whose EX-date == target_date ('YYYY-MM-DD' ET), else None.
    ...
    so a holder is not shown a paper loss for cash they actually receive. Mirrors HK update.py.
    """
    base = ticker.strip().upper()
    # Yahoo uses a dash for share classes (BRK.B -> BRK-B); try the as-is symbol first.
    candidates = []
    for c in [base, base.replace(".", "-")]:
    ...
    for cand in candidates:
        ...
        for ev in divs.values():
            ...
            if ts is None or amt is None:
                continue
            ex_date = datetime.fromtimestamp(ts, ET).strftime("%Y-%m-%d")
...
def fetch_dividends_today(positions: list, target_date: str) -> dict:
    """Return { ticker: dividend_per_share } for held US tickers going ex-div today."""
    out = {}
    for p in positions:
        ticker = p["ticker"].strip().replace(".HK", "").upper()
        div = _yahoo_dividend_for(ticker, target_date)
        if div:
            out[ticker] = div
            print(f"  $$ {ticker}: ex-dividend {div} USD/share today -> daily move shown on total-return basis")
    if not out:
...
def update_portfolio(db, doc_ref, user_id: str, today: str, tv_prices: dict):
    ...
    for p in positions:
        ...
        # change / previousClose == changePercent still holds. Mirrors HK update.py.
        div_today = dividends_today.get(ticker)
        ...
        print(f"  OK {ticker}: {price} (prevClose: {prev_close}, chg: {change_pct}%){div_note}")
    ...
    # dailyPnL uses TradingView's change_abs * quantity (authoritative source).
    # Yesterday's stored closingPrices can be slightly off if the cron captured
    # an intraday/auction price rather than settlement, which historically caused
    # divergence between calendar and Performance tab. Mirrors HK update.py fix.
    daily_pnl = 0
    yesterday_closing = yesterday_snap.get("closingPrices", {}) if yesterday_snap else {}
    for p in positions:
        ticker = p["ticker"]
        cur_price = p.get("currentPrice", p.get("entryPrice", 0))
        tv_entry = tv_prices.get(ticker) or {}
        tv_change_abs = tv_entry.get("changeAbs")
